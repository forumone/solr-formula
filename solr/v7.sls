...
create-solr-core:
  cmd:
    - cwd: /opt/solr
    - names:
      - bash bin/solr create -c {{ salt['pillar.get']('siteuser:0', 'vagrant') }}
    - run
    - user: solr

/var/solr/data/{{ salt['pillar.get']('siteuser:0', 'vagrant') }}:
  file.recurse:
...
usermodsolr{{ salt['pillar.get']('siteuser:0', 'vagrant') }}:
  cmd.run:
    - name: usermod -a -G solr {{ salt['pillar.get']('siteuser:0', 'vagrant') }}
